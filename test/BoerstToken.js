...
contract('BoerstToken', function(accounts) {
    ...
    it('initializes the contract with the correct values', function() {
        return BoerstToken.deployed().then(function(instance) {
            ...
            assert.equal(name, 'Boerst Token', 'has the correct name');
            return tokenInstance.symbol();
        }).then(function(symbol) {
            assert.equal(symbol, 'BOERST', 'has the correct symbol');
            return tokenInstance.standard();
        }).then(function(standard) {
            assert.equal(standard, 'Boerst Token v1.0', 'has the correct standard');
        });
    });
    ...
    it('transfers token ownership', function() {
        return BoerstToken.deployed().then(function(instance) {
            ...
            assert.equal(receipt.logs[0].args._tokens, 250000, 'logs the transfer amount');
    ...
    it('approves tokens for delegated transfer', function() {
        return BoerstToken.deployed().then(function(instance) {
            ...
            assert.equal(receipt.logs[0].args._tokenOwner, accounts[0], 'logs the account the tokens are authorized by');
            assert.equal(receipt.logs[0].args._spender, accounts[1], 'logs the account the tokens are authorized to');
            assert.equal(receipt.logs[0].args._tokens, 100, 'logs the transfer amount');
    ...
    it('handles delegated token transfers', function() {
        return BoerstToken.deployed().then(function(instance) {
            ...
            assert.equal(receipt.logs[0].args._tokens, 10, 'logs the transfer amount');
